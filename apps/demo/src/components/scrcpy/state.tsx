...
export class ScrcpyPageState {
    ...
    start = async () => {
        ...
        try {
            ...
            const { audioCodecOptions: ass, ...opt } = SETTING_STATE.settings;
            const options = new AdbScrcpyOptionsLatest(
                new ScrcpyOptionsLatest({
                    ...
                    // 车机的videoEncoder需要指定"OMX.google.h264.encoder"可正常显示
                    videoEncoder: "OMX.google.h264.encoder",
                    videoCodec: "h264",
                }),
            );
            // options.value.videoBitRate = 1_000_000;
            console.log(
                `--options--`,
                options,
                new ScrcpyOptionsLatest({
            ...
            console.log("AdbScrcpyClient.start");

            client.stdout.pipeTo(
                new WritableStream<string>({
                    write: action((line) => {
                        if (line.indexOf('"action":7') > -1) {
                            return;
                        }
                        this.log.push(line);
                        console.log("stdout write", line);
                    }),
                }),
            );
